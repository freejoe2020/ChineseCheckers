...
namespace Free.Checkers
{
    ...
    public class TQ_CheckerAIManagerV3 : ZFMonoBehaviour, ICheckerAIManager
    {
        ...
        [Tooltip("UCT exploration constant (��2 �� 1.414 is standard)")]
        ...
        private void CommitMove(TQAI_AIMove aiMove)
        {
            ...
            try
            {
                ...
                DebugLog($"AI move committed - Piece: ({aiMove.piece.CurrentCell.Q},{aiMove.piece.CurrentCell.R}) �� Target: ({aiMove.targetCell.Q},{aiMove.targetCell.R})");
            }
            catch (System.Exception ex)
            {
        ...
        private bool ValidateRealBoardMove(TQ_ChessPieceModel realPiece, TQ_HexCellModel realTargetCell, TQ_HexBoardModel realBoard)
        {
            ...
            try
            {
                ...
                if (!isValid)
                {
                    DebugLogWarning($"Move validation failed: Piece ({realPiece.CurrentCell.Q},{realPiece.CurrentCell.R}) �� Target ({realTargetCell.Q},{realTargetCell.R}) is not a valid move");
                }
